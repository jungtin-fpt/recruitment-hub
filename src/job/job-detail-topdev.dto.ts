...
export class JobDetailTopDevDTO {
	constructor(
		public url: string,
		public title: string,
    ...
    function formatLevel(level: string): LEVEL {
        if(level.includes("Nhân viên") || level.includes("Fresher")) level = "Nhân viên";
        switch(level) {
            case 'Thực tập sinh':
                return LEVEL.INTERN;
            case 'Nhân viên':
                return LEVEL.STAFF;
            case 'Nhân viên, Trưởng Nhóm':
                return LEVEL.STAFF;    
            case 'Trưởng nhóm':
                return LEVEL.HEAD;
            case 'Chuyên viên':
    ...
    function getSalaryConcurrency(salary: string): 'vnd' | 'usd' | 'none' {
        ...
        if (salary.includes('thuong luong')) return 'none';
