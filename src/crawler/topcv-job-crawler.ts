...
import { JobDetailRawTopCvDTO } from '../job/job-detail-raw-topcv-dto';
import { JobOverallDTO } from '../job/job-overall-dto';
import logger from '../logger';
import { ICrawler } from './icrawler';

export class TopCVCrawler implements ICrawler {
	async crawl(
		keyword: string,
		...
		try {
			...
			const browser = await launchBrowser(baseUrl, headless);
			let page = await goto(browser, searchUrl);
			page = await search(page, keyword);
			...
			for (let i = 0; i < jobs.length; i++) {
				...
				try {
					...
					if (job.url.includes('topcv.vn/viec-lam'))
						detail = await crawlJobDetail(page, job);
					else if (job.url.includes('topcv.vn/brand'))
						detail = await crawlJobDetailForBrandPage(page, job);

					if (detail) {
						logger.info('Crawl successfully job: ');
						logger.info(detail);
...
*/
async function goto(browser: Browser, url: string, page?: Page): Promise<Page> {
	try {
...
async function search(page: Page, keyword: string): Promise<Page> {
	try {
...
*/
async function crawlAllJobs(page: Page, totalPage?: number): Promise<JobOverallDTO[]> {
	try {
...
*/
async function crawlJobDetail(page: Page, job: JobOverallDTO): Promise<JobDetailDTO> {
	try {
		...
		const company = await crawlCompany(page, companyLink);
		const jobDetailRaw = new JobDetailRawTopCvDTO(
			job.url,
			job.title,
			description,
			region,
		...
		console.log(jobDetailRaw.reformat());
...
*/
async function crawlJobDetailForBrandPage(page: Page, job: JobOverallDTO): Promise<JobDetailDTO> {
	try {
		...
		const skills = await page.$eval('div.job-detail-section div.section-body', el => {
			const elements = Array.from(el.querySelectorAll('h4')).filter(el => el.textContent === 'Kỹ năng');
		...
		const company = await crawlCompanyForBrandPage(page, companyLink);
		const jobDetailRaw = new JobDetailRawTopCvDTO(
			job.url,
			job.title,
			description,
...
async function crawlCompany(page: Page, companyLink: string): Promise<CompanyDTO> {
	try {
		...
		logger.error(err);
		throw new Error(`Fail to crawl company: ${companyLink}`);
	}
}

async function crawlCompanyForBrandPage(page: Page, companyLink: string): Promise<CompanyDTO> {
	try {
		await page.goto(companyLink);
		await page.waitForSelector('div#company-name');
		const companyName = await page.$eval(
		...
		logger.error(err);
		throw new Error(`Fail to crawl company for brand page: ${companyLink}`);
	}
}